# My Offensive Security Portfolio

## Completed Assessments

### Week 1: SQL Injection Vulnerability Assessment
- **Link:** [SQL Injection Lab](./SQL-Injection-Lab/README.md)
- **Date:** September 12, 2026
- **Severity:** High
- **Status:** Completed & Documented
- **Skills Demonstrated:** 
  - SQL injection exploitation
  - Burp Suite proficiency
  - Vulnerability analysis
  - Professional reporting

---

### Week 2: Authentication Bypass Vulnerabilities
- **Link:** [Authentication Bypass Lab](./Authentication-Bypass-Lab/README.md)
- **Date:** 16th September 2026
- **Severity:** High
- **Labs:** 3 (Username enumeration, 2FA bypass, Password reset)
- **Status:** ✅ Completed & Documented
- **Skills Demonstrated:**
  - Username enumeration via response analysis
  - Multi-factor authentication bypass
  - Password reset token exploitation
  - Burp Suite Intruder automation
  - Burp Suite Repeater manipulation
 
---

## Progress Tracker

| Week | Topic | Status | Link |
|------|-------|--------|------|
| Week 1 | SQL Injection | Complete | [View](./SQL-Injection-Lab/README.md) |
| Week 2 | Authentication Bypass | [View](./Authentication-Bypass-Lab/README.md) |
| Week 3-4 | Advanced Web Vulnerabilities | In progress | - |

---

Last Updated: September 16, 2026
